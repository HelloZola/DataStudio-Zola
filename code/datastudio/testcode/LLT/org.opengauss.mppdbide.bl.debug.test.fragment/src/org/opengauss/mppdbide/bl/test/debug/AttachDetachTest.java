...
//        preparedstatementHandler = connection.getPreparedStatementResultSetHandler();
		try {
			...
			profileId = DBConnProfCache.getInstance().initConnectionProfile(serverInfo, status);
			...
			debugObject = new DebugObjects(6, "func", OBJECTTYPE.PLSQLFUNCTION, connectionProfile);
			...
			nestedDebugObject = new DebugObjects(7, "nestedfunc", OBJECTTYPE.PLSQLFUNCTION, connectionProfile);
		 ...
		 */
			...
			fail("Presetup for breakpoint testcase failed. Throws exception " + "PLSQLIDEException : "
					+ e.getMessage());
		} catch (Exception e) {
	...
	private void prepareConnectionResultSets() {
		...
		MockResultSet listnerResultSet = preparedstatementHandler.createResultSet();
		listnerResultSet.addRow(new Integer[] { 12 });
		preparedstatementHandler.prepareResultSet("SELECT pldbg_create_listener();", listnerResultSet);

		MockResultSet initializeDebugResult = preparedstatementHandler.createResultSet();
		initializeDebugResult.addRow(new String[] { "1231" });
		preparedstatementHandler.prepareResultSet("SELECT pldbg_initialize_debug();", initializeDebugResult);

		MockResultSet attachResult = preparedstatementHandler.createResultSet();
		attachResult.addRow(new Boolean[] { true });

		preparedstatementHandler.prepareResultSet("SELECT pldbg_attach_session(?,?);", attachResult);

		MockResultSet detachResult = preparedstatementHandler.createResultSet();
		detachResult.addRow(new Boolean[] { true });
		preparedstatementHandler.prepareResultSet("SELECT pldbg_detach_session(?);", detachResult);

		MockResultSet isDebugResult = preparedstatementHandler.createResultSet();
		isDebugResult.addRow(new Boolean[] { true });

		preparedstatementHandler.prepareResultSet("SELECT pldbg_is_debug_on();", isDebugResult);

		MockResultSet debugOn = preparedstatementHandler.createResultSet();
		debugOn.addRow(new Boolean[] { true });
		preparedstatementHandler.prepareResultSet("SELECT pldbg_debug_on();", debugOn);

		MockResultSet debugOff = preparedstatementHandler.createResultSet();
		debugOff.addRow(new Boolean[] { true });
		preparedstatementHandler.prepareResultSet("SELECT pldbg_debug_off();", debugOff);
		CommonLLTUtilsHelper.prepareProxyInfo(preparedstatementHandler);
	...
	public void testTTA_BL_ATTACH002_FUNC_003_001() {
		preparedstatementHandler.prepareThrowsSQLException("SELECT pldbg_attach_session(?,?);");

		try {
			if (isEnvSupprot()) {
				connectionProfile = prepareSetup();
				((Executor) connectionProfile.getExecutor()).connectToServer(serverInfo,
						connectionProfile.getConnectionManager().getConnectionDriver());
	...
	public void testTTA_BL_ATTACH002_FUNC_003_002() {
		...
		preparedstatementHandler.prepareThrowsSQLException("SELECT pldbg_attach_session(?,?);", sqlException);
		try {
			if (isEnvSupprot()) {
				connectionProfile = prepareSetup();
				((Executor) connectionProfile.getExecutor()).connectToServer(serverInfo,
